...
public class Portfolio {
	...
	public boolean buyAdditionalStock(double percent, Stock stock) {
		...
		double totalCost = quantity * stockService.getPrice(stock);
	...
	public double todaysStockEarnings()
	{
		double earnings = 0;
		
		for(Stock stock : this.stocks)
		{
			earnings += stock.getQuantity()*(this.stockService.getClosingPrice(stock) - this.stockService.getOpenPrice(stock));
		}
